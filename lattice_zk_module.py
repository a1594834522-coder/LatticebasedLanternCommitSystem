...
# Lantern-based RLWE encryption utilities
def sample_uniform_poly():
    """Sample a polynomial uniformly from R_q."""
    return R_q.random_element()

def sample_small_poly(bound=1):
    """Sample a small polynomial with coefficients in [-bound, bound]."""
    coeffs = [random.randint(-bound, bound) for _ in range(d)]
    return R_q([c % q for c in coeffs])

def encode_message_bits(bits):
    """Embed up to d message bits as a polynomial scaled into R_q."""
    if len(bits) > d:
        raise ValueError("Message too long for current ring dimension.")
    coeffs = [0] * d
    half_q = q // 2
    for i, bit in enumerate(bits):
        if bit not in (0, 1):
            raise ValueError("Message must be binary.")
        coeffs[i] = half_q if bit else 0
    return R_q(coeffs)

def decode_message_bits(poly, length):
    """Recover a binary message from a decrypted polynomial."""
    if length > d:
        raise ValueError("Requested length exceeds ring dimension.")
    coeffs = poly.list()
    half_q = q // 2
    bits = []
    for i in range(length):
        centered = Zq_2_ZZ(int(coeffs[i]))
        distance_to_zero = abs(centered)
        distance_to_one = abs(abs(centered) - half_q)
        bits.append(1 if distance_to_one < distance_to_zero else 0)
    return bits

def lantern_keygen(bound=1):
    """Generate a Lantern-style RLWE key pair."""
    a = sample_uniform_poly()
    s = sample_small_poly(bound)
    e = sample_small_poly(bound)
    b = a * s + e
    return {'a': a, 'b': b}, s

def lantern_encrypt(pubkey, bits, bound=1):
    """Encrypt a binary message using the RLWE public key."""
    message_poly = encode_message_bits(bits)
    r = sample_small_poly(bound)
    e1 = sample_small_poly(bound)
    e2 = sample_small_poly(bound)
    u = pubkey['a'] * r + e1
    v = pubkey['b'] * r + e2 + message_poly
    return u, v

def lantern_decrypt(secret, ciphertext, length):
    """Decrypt a ciphertext tuple and recover the original bits."""
    u, v = ciphertext
    message_poly = v - u * secret
    return decode_message_bits(message_poly, length), message_poly

if __name__ == "__main__":
    # Demo: round-trip encryption of a short bitstring
    example_bits = [1, 0, 1, 1, 0]
    pubkey, seckey = lantern_keygen(bound=2)
    ciphertext = lantern_encrypt(pubkey, example_bits, bound=2)
    recovered_bits, recovered_poly = lantern_decrypt(seckey, ciphertext, len(example_bits))
    print('Plaintext bits :', example_bits)
    print('Recovered bits :', recovered_bits)
    print('Round-trip OK? ', recovered_bits == example_bits)
